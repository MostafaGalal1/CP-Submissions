...
int main() {
    ...
    while (t--) {
        ...
        auto it = lower_bound(a.begin(), a.end(), make_pair(LONG_LONG_MAX, 0ll));
        if (it != a.end()) {
            ll cnt = 0, mxi = it->second, st = it->second, mx = 0;
            for (int i = (int)(it-a.begin()); i < (ll)a.size()-1; ++i) {
                if (a[i+1].second - a[i].second == 1)
                    cnt++;
                else
                    cnt = 0, st = a[i+1].second;
                if (mx < cnt){
                    mx = cnt;
                    mxi = st;
                }
            }
            cout << mxi << ' ' << mxi + mx << endl;
        } else {
            cout << -1 << endl;
        }
    }
