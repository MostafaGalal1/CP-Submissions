...
//    t = 1;
    ...
    while (t --> 0) {
        ...
        for (int i = 0; i < k; ++i) {
            cin >> nu;
            auto it = s.lower_bound({nu, 0});
            if (it != s.end() && it->first == nu) {
                for (int j = 0; j < (ll)wait.size(); ++j)
                    res.push_back(it->second+1);
                wait.clear();
                res.push_back(it->second+1);
                mapp[a[it->second]].erase(it->second);
                mapp[nu].insert(it->second);
                a[it->second] = nu;
                s.erase(it);
            } else if (!mapp[nu].empty()){
                for (int j = 0; j < (ll)wait.size(); ++j)
                    res.push_back(*mapp[nu].begin()+1);
                wait.clear();
                res.push_back(*mapp[nu].begin()+1);
            } else {
                wait.push_back(nu);
