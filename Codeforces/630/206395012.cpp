...
//    while (t --> 0){
    ...
    ld tri_ang = 180.0 * 3 / n;
    ld cir_ang = 360.0 / n;

    if (n == 5)
        tri_ang = 108;

    ld area_under_chord = PI * r * r / n - r * r * sin(rd(cir_ang)) * 0.5;
    ld rem_ang = (180 - tri_ang) / 2;
    ld len = 2 * r * sin(rd(cir_ang)/2);
    ld side_len = sin(rd(rem_ang)) * len / sin(rd(tri_ang));
    ld area_tri = 0.5 * side_len * side_len * sin(rd(tri_ang));
    ld cir_area = PI * r * r;

    cout << setprecision(12) << (cir_area - n * (area_under_chord + area_tri)) << endl;
//    }
    return 0;
}