...
int main() {
    ...
    while (t --> 0){
        ...
        ll x, y;
        ll mxy = LONG_LONG_MIN, mny = LONG_LONG_MAX, mxx = LONG_LONG_MIN, mnx = LONG_LONG_MAX;
        ...
        ll res = 0;
        if (mny != LONG_LONG_MAX)
            res += abs(mny);
        if (mnx != LONG_LONG_MAX)
            res += abs(mnx);
        if (mxy != LONG_LONG_MIN)
            res += abs(mxy);
        if (mxx != LONG_LONG_MIN)
            res += abs(mxx);
