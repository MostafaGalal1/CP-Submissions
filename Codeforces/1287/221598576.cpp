            ...
            for (int j = i+1; j < n; ++j) {
                ...
                for (int l = 0; l < k; ++l) {
                    if (r[i][l] == r[j][l])
                        ...
                        s += c[3 - ch[r[i][l]] - ch[r[j][l]]];
                }
                if (lower_bound(r.begin(), r.end(), s) - r.begin() < j)
                    continue;
                res += upper_bound(r.begin()+j+1, r.end(), s) - lower_bound(r.begin()+j+1, r.end(), s);
            }
        }
