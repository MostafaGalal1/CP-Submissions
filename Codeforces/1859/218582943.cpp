...
//    t = 1;
    ...
    while (t --> 0) {
        ...
        for (int i = 0; i < n; ++i) {
            if (s.empty())
                s.insert(v[i]);
            else if (prev(s.end())->second < v[i].first){
                s.insert(v[i]);
            } else if (prev(s.end())->second < v[i].second){
                ll st = prev(s.end())->first;
                s.erase(prev(s.end()));
                s.insert({st, v[i].second});
            }
        }
        ...
        while (q --> 0){
            cin >> p;
            auto it = s.lower_bound(make_pair(p, LONG_LONG_MAX));
            if (it == s.begin())
                cout << p << ' ';
            else {
                if (prev(it)->second >= p)
