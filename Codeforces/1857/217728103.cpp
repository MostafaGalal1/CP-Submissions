...
//    t = 1;
    ...
    while (t --> 0) {
        ...
        sort(a.begin(), a.end());
        res[0].first = 0, res[0].second = a[0].second;

        for (int i = 0; i < n-1; ++i) {
            dif[i] = a[i+1].first - a[i].first + (i?0:1), res[i+1].second = a[i+1].second;
        }

        for (int i = 0; i < n-1; ++i) {
            res[0].first += dif[i] * (n-1-i);
        }

        ll neg = -(n-2);
        for (int i = 1; i < n; ++i) {
            res[i].first += res[i-1].first + neg * (dif[i-1] - (i==1?1:0));
            neg += 2;
        }

        std::sort(res.begin(), res.end(), compareSecondElement);
        for (int i = 0; i < n; ++i) {
            cout << res[i].first+1 << ' ';
        }
        cout << endl;
