...
//    t = 1;
    ...
    while (t --> 0) {
        ...
        for (int i = 0; i < q; ++i) {
            cin >> x >> y;
            ld b = -x, c = y;
            pair<ld, ld> roots = solveQuadraticEquation(1, b, c);
            if (roots.first == roots.second && abs(roots.first - (ll)roots.first) < 1e-4){
                auto it = lower_bound(a.begin(), a.end(), roots.first);
                auto is = upper_bound(a.begin(), a.end(), roots.first);
                ll dif = is - it;
                cout << dif * (dif-1) / 2 << ' ';
            } else if (abs(roots.first - (ll)roots.first) < 1e-9 && abs(roots.second - (ll)roots.second) < 1e-4){
                auto it = lower_bound(a.begin(), a.end(), (ll)roots.first);
                auto is = upper_bound(a.begin(), a.end(), (ll)roots.first);
                auto itt = lower_bound(a.begin(), a.end(), (ll)roots.second);
                auto iss = upper_bound(a.begin(), a.end(), (ll)roots.second);
                ll dif = is - it, diff = iss - itt;
                cout << dif * diff << ' ';
            } else
                cout << 0 << ' ';
        }
