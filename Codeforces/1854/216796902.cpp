...
//    t = 1;
    ...
    while (t --> 0) {
        ...
        for (int i = neg; i > 0; --i) {
            if (a[i-1] > a[i])
                a[i-1] += a[i], res.emplace_back(i, i+1);
        }

        for (int i = neg+1; i < n-1; ++i) {
            if (a[i+1] < a[i] && a[i+1] >= 0)
                a[i+1] += a[i], res.emplace_back(i+2, i+1);
            else if (a[i+1] < 0) {
                ll nu = a[i];
                while (nu + a[i+1] < (nu+1)/2)
                    nu *= 2, res.emplace_back(i+1, i+1);
                a[i] = nu;
                a[i+1] += a[i], a[i+1] *= 2, res.emplace_back(i+2, i+1), res.emplace_back(i+2, i+2);
            }
        }
