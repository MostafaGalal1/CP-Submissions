...
int main() {
    ...
    while (t --> 0) {
        ...
        vector<ll> l(k), r(k), c(k, 0);
        vector<set<pair<ll, pair<ll, ll>>>> ss(k);
        for (int i = 0; i < k; ++i) {
            cin >> l[i];
        }
        for (int i = 0; i < k; ++i) {
        ...
        for (int i = 0; i < (ll)qs.size(); ++i) {
            x = qs[i];
            auto it = lower_bound(r.begin(), r.end(), x);
            ll ind = it - r.begin();
            ss[ind].insert({max(x, r[ind] + l[ind] - x) - min(x, r[ind] + l[ind] - x) + 1,{min(x, r[ind] + l[ind] - x) - 1, max(x, r[ind] + l[ind] - x)}});
            mp[{min(x, r[ind] + l[ind] - x) - 1, max(x, r[ind] + l[ind] - x)}]++;
        }

        for (int i = 0; i < k; ++i) {
            vector<pair<ll, ll>> p;
            for (auto& sm : ss[i])
                p.push_back(sm.second);
            for (int j = (ll)p.size() - 2; j >= 0; --j)
                mp[p[j]] += mp[p[j+1]];
            for (int j = 0; j < (ll)p.size(); ++j) {
                if (mp[p[j]]%2) {
                    if (p[j] == p[0])
                        reverse(s.begin() +  p[j].first, s.begin() +  p[j].second);
                    else {
                        for (int m = p[j].first; m < p[j - 1].first; ++m)
                            swap(s[m], s[p[j].second-1+p[j].first-m]);
                    }
                }
            }
        }
