...
int main() {
    ...
    while (t --> 0){
        ...
        ll res = 0;
        for (auto & ch : k){
            vector<ll> d = divis[ch.first];
            res += ch.second * (ch.second - 1) * (ch.second - 2);
            for (int i = 1; i < d.size() && d[i] * ch.first <= mx; ++i) {
                res += k[ch.first/d[i]] * ch.second * k[ch.first*d[i]];
                if (k[ch.first/d[i]] == 0)
                    k.erase(ch.first/d[i]);
                if (k[ch.first*d[i]] == 0)
                    k.erase(ch.first*d[i]);
            }
        }
