...
int main() {
    ...
    for (int i = 0; i < n; ++i)
        cin >> a[i].first, a[i].second = i+1, s+=a[i].first, m[a[i].first]++;

    sort(a.begin(), a.end());
    vector<ll> res;
    for (int i = 0; i < n; ++i) {
        if ((s-a[i].first)%2 == 0) {
            auto it = lower_bound(a.begin(), a.end(), make_pair((s - a[i].first) / 2, 0ll));
            if (it != a.end() && it->first == (s - a[i].first) / 2 && (it->second != a[i].second || m[(s - a[i].first) / 2] > 1))
