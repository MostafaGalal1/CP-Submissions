...
//    while (t --> 0){
    ...
    vector<vector<ll>> dp(2, vector<ll>(n+2, LONG_MAX));
    dp[0][0] = dp[1][0] = dp[0][n+1] = dp[1][n+1] = 0;

    for (int i = 0; i <= n; ++i) {
        dp[1][i+1] = dp[1][i] + (islower(s[i])?1:0);
        dp[0][n-i] = dp[0][n+1-i] + (isupper(s[n-1-i])?1:0);
    }

    ll res = n;
    for (int i = 0; i <= n+1; ++i)
        res = min(res, dp[1][i] + dp[0][i+1]);
