        ...
        for (ll i = 0; i < n; ++i) {
            for (ll j = i; j < n; ++j) {
                if (a[i].first * a[j].first > 2 * n)
                    break;
                auto it = lower_bound(a.begin()+i+1, a.end(), make_pair(a[j].first, a[i].first*a[j].first-a[i].second));
                if (it != a.end() && it->first == a[j].first && it->second == a[i].first*a[j].first-a[i].second)
                    cnt++;
            }
        }
        cout << cnt << endl;
