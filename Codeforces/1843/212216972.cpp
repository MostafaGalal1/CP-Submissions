...
ll dfs(ll node, vector<vector<ll>>& a, vector<bool>& vis, vector<ll>& in){
    vis[node] = true;
    ll cnt = 1;
    vector<pair<ll, ll>> p;
    ...
    for (ll child : a[node]){
        if (!vis[child])
            p.emplace_back(in[child]-1, child);
    }

    sort(p.begin(), p.end());
    if (p.size()>1) {
        if (p[0].first == p[1].first)
            cnt += min(dfs(p[0].second, a, vis, in), dfs(p[1].second, a, vis, in));
        else
            cnt += dfs(p[0].second, a, vis, in);
    }
    if (!p.empty())
        cnt++;
    return cnt;
...
bool func(ll n, ll m, vector<pair<ll, ll>>& a, vector<ll>& q){
    vector<ll> p(n, 0);
    for (int i = 0; i <= m; ++i)
        p[q[i]] = 1;
    for (int i = 1; i < n; ++i)
    ...
    for (int i = 0; i < (ll)a.size(); ++i) {
        if (a[i].second - a[i].first + 1 < (p[a[i].second] - (a[i].first?p[a[i].first-1]:0)) * 2)
            return true;
    }
    return false;
}
