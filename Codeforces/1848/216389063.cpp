...
//    t = 1;
    ...
    while (t --> 0) {
        ll n, k;
        cin >> n >> k;
        vector<ll> a(n), cur(k+1, -1), pre(k+1);
        vector<ll> mxc(k+1, 0), mxb(k+1, -1);

        for (int i = 0; i < n; ++i) {
            cin >> a[i];
            pre[a[i]] = cur[a[i]];
            cur[a[i]] = i;
            if (cur[a[i]] - pre[a[i]] - 1 > mxc[a[i]]) {
                mxb[a[i]] = mxc[a[i]];
                mxc[a[i]] = cur[a[i]] - pre[a[i]] - 1;
            } else if (cur[a[i]] - pre[a[i]] - 1 > mxb[a[i]])
                mxb[a[i]] = cur[a[i]] - pre[a[i]] - 1;
        }
        for (int i = 1; i <= k; ++i) {
            pre[i] = cur[i];
            cur[i] = n;
            if (cur[i] - pre[i] - 1 > mxc[i]) {
                mxb[i] = mxc[i];
                mxc[i] = cur[i] - pre[i] - 1;
            } else if (cur[i] - pre[i] - 1 > mxb[i])
                mxb[i] = cur[i] - pre[i] - 1;
        }

        ll mn = LONG_MAX;
        for (int i = 1; i <= k; ++i)
            mn = min(max(mxb[i], mxc[i]/2), mn);
        cout << mn << endl;
    }
    return 0;
}