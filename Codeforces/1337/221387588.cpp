...
ll dfs(ll node, ll cnt, vector<vector<ll>>& adj, vector<bool>& vis, vector<ll>& con, vector<ll>& cld) {
    vis[node] = true;
    con[node] = cnt;
    for (auto ch : adj[node]) {
        if (!vis[ch])
            cld[node] += dfs(ch, cnt+1, adj, vis, con, cld);
    }
    return cld[node] + 1;
}
...
//    cin >> t;
    ...
    while (t --> 0) {
        ...
        vector<ll> con(n+1, 0), cld(n+1, 0), res;
        vector<bool> vis(n+1, false);
        dfs(1, 0, adj, vis, con, cld);

        for (int i = 1; i <= n; ++i)
            res.push_back(con[i] - cld[i]);
