...
void rec(ll k, ll n, ll x, vector<ll>& w, vector<ll>& v, vector<vector<ll>>& dp) {
    if (n == (ll)w.size()+1)
        return;
    dp[n][k] = x;

    rec(k, n+1, x, w, v, dp);
    if (n < (ll)w.size() && k + v[n] <= 1000 && x-w[n] >= 0)
        rec(k+v[n], n+1, x-w[n], w, v, dp);
}

ll rec(ll n, ll m, vector<vector<ll>>& dp){
    if (n == m)
        return dp[n][m] = 0;
    if (dp[n][m] != LONG_MAX)
        return dp[n][m];
    for (int i = 1; i <= n/2; ++i)
        dp[n][m] = min(rec(i, m, dp) + rec(n-i, m, dp), dp[n][m]);
    for (int i = 1; i <= m/2; ++i)
        dp[n][m] = min(rec(n, i, dp) + rec(n, m-i, dp), dp[n][m]);
    return ++dp[n][m];
}
...
//    cin >> t;
    ...
    while (t--) {
        ll n, m;
        cin >> n >> m;
        vector<vector<ll>> dp(501, vector<ll>(501, LONG_MAX));
