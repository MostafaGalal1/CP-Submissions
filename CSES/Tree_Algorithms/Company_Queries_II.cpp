...
void lca(ll node, ll l, vector<vector<ll>>& g, vector<ll>& depth, vector<vector<ll>>& up){
    for (ll ch : g[node]){
...
ll ancestor(ll a, ll b, ll l, vector<vector<ll>>& up, vector<ll>& depth){
    if (depth[a] < depth[b])
        swap(a, b);
...
//    cin >> t;
    ...
    while (t --> 0) {
        ...
        for (int i = 0; i < q; ++i) {
            cin >> x >> y;
            cout << ancestor(x, y, l, up, depth) << endl;
        }
    }
    return 0;
}